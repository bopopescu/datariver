...
type fields struct {
	...
	fieldsSnake map[string]*fieldInfo
}

// add field info
func (f *fields) Add(fi *fieldInfo) (added bool) {
	add := false
	snake := snakeString(fi.name)
	if fi.column == "" {
		fi.column = snake
	}
	if f.fields[fi.name] == nil && f.columns[fi.column] == nil && f.fieldsSnake[snake] == nil {
		f.columns[fi.column] = fi
		f.fields[fi.name] = fi
		f.fieldsSnake[snake] = fi
		add = true
	}
	return add
}
...
// get field info by name
func (f *fields) GetByNameSnake(name string) *fieldInfo {
	return f.fieldsSnake[name]
}

// get field info by column name
func (f *fields) GetByColumn(column string) *fieldInfo {
	if fi, ok := f.columns[column]; ok {
		return fi
	}
	if fi, ok := f.fieldsSnake[column]; ok {
		return fi
	}
	return nil
}
...
func newFields() *fields {
	...
	f.fieldsSnake = make(map[string]*fieldInfo)
	return f
}
...
checkType:
	...
	fi.column = sf.Tag.Get(defaultStructTagName) //getColumnName(fieldType, addrField, sf, tags["column"])

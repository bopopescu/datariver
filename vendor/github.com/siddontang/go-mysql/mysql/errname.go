...
var MySQLErrName = map[uint16]string{
	...
	ER_MASTER:                                        "Error from main: '%-.64s'",
	ER_MASTER_NET_READ:                               "Net error reading from main",
	ER_MASTER_NET_WRITE:                              "Net error writing to main",
	...
	ER_SLAVE_MUST_STOP:                               "This operation cannot be performed with a running subordinate; run STOP SLAVE first",
	ER_SLAVE_NOT_RUNNING:                             "This operation requires a running subordinate; configure subordinate and do START SLAVE",
	ER_BAD_SLAVE:                                     "The server is not configured as subordinate; fix in config file or with CHANGE MASTER TO",
	ER_MASTER_INFO:                                   "Could not initialize main info structure; more error messages can be found in the MySQL error log",
	ER_SLAVE_THREAD:                                  "Could not create subordinate thread; check system resources",
	...
	ER_CONNECT_TO_MASTER:                             "Error connecting to main: %-.128s",
	ER_QUERY_ON_MASTER:                               "Error running query on main: %-.128s",
	...
	ER_MASTER_FATAL_ERROR_READING_BINLOG:             "Got fatal error %d from main when reading data from binary log: '%-.320s'",
	ER_SLAVE_IGNORED_TABLE:                           "Subordinate SQL thread ignored the query because of replicate-*-table rules",
	...
	ER_SLAVE_WAS_RUNNING:                             "Subordinate is already running",
	ER_SLAVE_WAS_NOT_RUNNING:                         "Subordinate already has been stopped",
	...
	ER_SLAVE_IGNORED_SSL_PARAMS:                      "SSL parameters in CHANGE MASTER are ignored because this MySQL subordinate was compiled without SSL support; they can be used later if MySQL subordinate with SSL is started",
	...
	ER_MISSING_SKIP_SLAVE:                            "It is recommended to use --skip-subordinate-start when doing step-by-step replication with START SLAVE UNTIL; otherwise, you will get problems if you get an unexpected subordinate's mysqld restart",
	...
	ER_BINLOG_ROW_WRONG_TABLE_DEF:                    "Table definition on main and subordinate does not match: %s",
	ER_BINLOG_ROW_RBR_TO_SBR:                         "Subordinate running with --log-subordinate-updates must use row-based binary logging to be able to replicate row-based binary log events",
	...
	ER_SLAVE_INCIDENT:                                "The incident %s occured on the main. Message: %-.64s",
	...
	ER_SLAVE_MASTER_COM_FAILURE:                      "Main command %s failed: %s",
	...
	ER_NEVER_USED:                                    "Ambiguous subordinate modes combination. %s",
	...
	WARN_NO_MASTER_INFO:                                                 "The main info structure does not exist",
	...
	ER_SLAVE_HEARTBEAT_FAILURE:                                          "Unexpected main's heartbeat data: %s",
	...
	ER_SLAVE_IGNORE_SERVER_IDS:                                          "The requested server id %d clashes with the subordinate startup option --replicate-same-server-id",
	...
	ER_BINLOG_UNSAFE_SYSTEM_TABLE:                                       "The statement is unsafe because it uses the general log, slow query log, or performance_schema table(s). This is unsafe because system tables may differ on subordinates.",
	...
	ER_BINLOG_UNSAFE_UDF:                                                "Statement is unsafe because it uses a UDF which may not return the same value on the subordinate.",
	ER_BINLOG_UNSAFE_SYSTEM_VARIABLE:                                    "Statement is unsafe because it uses a system variable that may have a different value on the subordinate.",
	ER_BINLOG_UNSAFE_SYSTEM_FUNCTION:                                    "Statement is unsafe because it uses a system function that may return a different value on the subordinate.",
	...
	ER_SLAVE_HEARTBEAT_VALUE_OUT_OF_RANGE_MAX:                           "The requested value for the heartbeat period exceeds the value of `subordinate_net_timeout' seconds. A sensible value for the period should be less than the timeout.",
	...
	ER_BINLOG_UNSAFE_INSERT_IGNORE_SELECT:                               "INSERT IGNORE... SELECT is unsafe because the order in which rows are retrieved by the SELECT determines which (if any) rows are ignored. This order cannot be predicted and may differ on main and the subordinate.",
	ER_BINLOG_UNSAFE_INSERT_SELECT_UPDATE:                               "INSERT... SELECT... ON DUPLICATE KEY UPDATE is unsafe because the order in which rows are retrieved by the SELECT determines which (if any) rows are updated. This order cannot be predicted and may differ on main and the subordinate.",
	ER_BINLOG_UNSAFE_REPLACE_SELECT:                                     "REPLACE... SELECT is unsafe because the order in which rows are retrieved by the SELECT determines which (if any) rows are replaced. This order cannot be predicted and may differ on main and the subordinate.",
	ER_BINLOG_UNSAFE_CREATE_IGNORE_SELECT:                               "CREATE... IGNORE SELECT is unsafe because the order in which rows are retrieved by the SELECT determines which (if any) rows are ignored. This order cannot be predicted and may differ on main and the subordinate.",
	ER_BINLOG_UNSAFE_CREATE_REPLACE_SELECT:                              "CREATE... REPLACE SELECT is unsafe because the order in which rows are retrieved by the SELECT determines which (if any) rows are replaced. This order cannot be predicted and may differ on main and the subordinate.",
	ER_BINLOG_UNSAFE_UPDATE_IGNORE:                                      "UPDATE IGNORE is unsafe because the order in which rows are updated determines which (if any) rows are ignored. This order cannot be predicted and may differ on main and the subordinate.",
	...
	ER_BINLOG_UNSAFE_CREATE_SELECT_AUTOINC:                              "CREATE TABLE... SELECT...  on a table with an auto-increment column is unsafe because the order in which rows are retrieved by the SELECT determines which (if any) rows are inserted. This order cannot be predicted and may differ on main and the subordinate.",
	...
	ER_MASTER_DELAY_VALUE_OUT_OF_RANGE:                                  "The requested value %u for the main delay exceeds the maximum %u",
	...
	ER_MTS_FEATURE_IS_NOT_SUPPORTED:                                     "%s is not supported in multi-threaded subordinate mode. %s",
	...
	ER_INSECURE_CHANGE_MASTER:                                           "Storing MySQL user name or password information in the main.info repository is not secure and is therefore not recommended. Please see the MySQL Manual for more about this issue and possible alternatives.",
	...
	ER_SQLTHREAD_WITH_SECURE_SLAVE:                                      "Setting authentication options is not possible when only the Subordinate SQL Thread is being started.",
	...
	ER_GTID_MODE_REQUIRES_BINLOG:                                        "@@GLOBAL.GTID_MODE = ON or UPGRADE_STEP_1 or UPGRADE_STEP_2 requires --log-bin and --log-subordinate-updates.",
	...
	ER_MASTER_HAS_PURGED_REQUIRED_GTIDS:                                 "The subordinate is connecting using CHANGE MASTER TO MASTER_AUTO_POSITION = 1, but the main has purged binary logs containing GTIDs that the subordinate requires.",
	...
	ER_SLAVE_CONFIGURATION:                                              "Subordinate is not configured or failed to initialize properly. You must at least set --server-id to enable either a main or a subordinate. Additional error messages can be found in the MySQL error log.",
	...
	ER_MTS_CHANGE_MASTER_CANT_RUN_WITH_GAPS:                             "CHANGE MASTER cannot be executed when the subordinate was stopped with an error or killed in MTS mode. Consider using RESET SLAVE or START SLAVE UNTIL.",
	...
	ER_SLAVE_SILENT_RETRY_TRANSACTION:                                   "Subordinate must silently retry current transaction",
	...
	ER_GTID_NEXT_TYPE_UNDEFINED_GROUP:                                   "When @@SESSION.GTID_NEXT is set to a GTID, you must explicitly set it again after a COMMIT or ROLLBACK. If you see this error message in the subordinate SQL thread, it means that a table in the current transaction is transactional on the main and non-transactional on the subordinate. In a client connection, it means that you executed SET @@SESSION.GTID_NEXT before a transaction and forgot to set @@SESSION.GTID_NEXT to a different identifier or to 'AUTOMATIC' after COMMIT or ROLLBACK. Current @@SESSION.GTID_NEXT is '%s'.",
	...
	ER_SQL_SLAVE_SKIP_COUNTER_NOT_SETTABLE_IN_GTID_MODE:                 "sql_subordinate_skip_counter can not be set when the server is running with @@GLOBAL.GTID_MODE = ON. Instead, for each transaction that you want to skip, generate an empty transaction with the same GTID as the transaction",
